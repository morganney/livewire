<?php
	/**
	 * Class for Motor Control part. Primary responsiblity is to allow
	...
	class MCPart extends Part {
		...
		protected function fetchDetails() {
			...
			$this->query(
				"SELECT {$relevant_fields_aliased} specs, weight, img, manuf, manuf_slug, website, affiliates, ns_new_id, ns_green_id, display, CONCAT('ATTACHMENTS/', subdir, '/', filename) AS catalog_uri 
		...
		public function getPageTitle() {
			...
			return "{$this->_part_no} {$sep} Motor Control {$sep} {$this->_details['manuf']} | {$branding}";
		}
		...
		public function getH1Txt() {
			...
			$cat_crumb = link_to('Motor Control', '@motor_control');
			$manuf_crumb = link_to($this->_details['manuf'], "@mc_manuf?manuf_slug={$this->_details['manuf_slug']}");
			return "<strong>{$this->_part_no}</strong> {$sep} {$cat_crumb} {$sep} {$manuf_crumb}";
		}
		
		public function getMetaKeywords() {
			return 'motor control,push buttons,relays,contactors,coils,heaters,din rail mount,enclosed starters,motor control center,feeder bucket,auxilary switch';
		}
