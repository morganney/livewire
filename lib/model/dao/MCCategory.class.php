<?php
	...
	class MCCategory extends DAO {
		...
		public function __construct($list_decorator_type = NULL) {
			parent::__construct();
			$this->_id = 'mc';
			$this->_specs_preview_fields = array('specs');
		...
		/**
		 * Method for retrieving motor control parts details for a particular manufacturer
		...
		public function fetchPartsListByManuf($manuf_id, $page_num) {
			...
			$this->query(
				"SELECT part_no, {$specs_preview_sql}, manuf, img, ns_new_id, ns_green_id, display, (
				 ...
				 WHERE cat_id='{$this->_id}' AND manuf_id={$manuf_id} 
				 ORDER BY display DESC, has_img DESC, part_no ASC 
				 LIMIT {$offset}, {$limit}"
			);
			if($this->queryOK()) {
				$parts = array();
		 ...
		 * creating motor control subcategories.
		 * 
		...
		/**
		 * Method for executing motor control subcategory creation logic.
		...
		/**
		 * Method for fetching the featured mc parts for a given manufacturer.
		...
		/**
		 * Extends the motor control parts array for a given manufacturer by adding
		...
		/**
		 * Responsible for building necessary SQL variables for motor control
